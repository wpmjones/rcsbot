...
async def quick_export(channel: discord.TextChannel,
    ...
    if guild:
        channel.guild = guild
    transcript = (await Transcript(channel=channel,
                                   limit=None,
                                   messages=None,
                                   pytz_timezone="UTC",
                                   military_time=True,
    ...
    if not transcript:
        return
    transcript_embed = discord.Embed(description=f"**Transcript Name:** transcript-{channel.name}\n\n",
...
async def export(channel: discord.TextChannel,
    ...
    if guild:
        channel.guild = guild
    return (await Transcript(channel=channel,
                             limit=limit,
                             messages=None,
...
async def raw_export(channel: discord.TextChannel,
...
async def quick_link(channel: discord.TextChannel, message: discord.Message):
    embed = discord.Embed(
        ...
        description=(f"[Click here to view the transcript](https://mahto.id/chat-exporter?url="
