...
async def error_handler(ctx, error):
    ...
    if isinstance(error, (checks.NoConfigFailure, paginator.CannotPaginate, commands.CheckFailure)):
        ctx.bot.logger.info("command raised checks failure: %s, author: %s", ctx.invoked_with, ctx.author.id)
    ...
    if isinstance(error, commands.CommandOnCooldown):
        ...
        ctx.bot.logger.info("command raised cooldown error: %s", ctx.invoked_with)
